...
static int __init init_sys_scanner(void)
{
	if (sysptr == NULL)
		sysptr = scanner;
	
	printk(KERN_INFO "installed new sys_scanner module %p %p\n", sysptr, &scanner);
	return SUCCESS;
}
static void  __exit exit_sys_scanner(void)
{
	if (sysptr != NULL)
		sysptr = NULL;
	printk(KERN_INFO "removed sys_xconcat module\n");
}
module_init(init_sys_scanner);
module_exit(exit_sys_scanner);
MODULE_LICENSE("GPL");
